...
const Layout = (props) => {
    ...
    const onMouseEnter = () => {
        if (!layoutState.anchored) {
            ...
            setLayoutState((prevLayoutState) => ({ ...prevLayoutState, sidebarActive: true }));
        }
    };
    ...
    const onMouseLeave = () => {
        if (!layoutState.anchored) {
            if (!timeout) {
                timeout = setTimeout(() => setLayoutState((prevLayoutState) => ({ ...prevLayoutState, sidebarActive: false })), 300);
            }
        }
    };
